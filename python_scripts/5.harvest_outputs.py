...
logging.info(f'Start 5.harvest_outputs.')
...
parser.add_argument('--binary-assoc-test', type=str, default=None, help='chi2|linear|logit, the association test '
                                                                        'performed for a dichotomous phenotype')
parser.add_argument('--skip-assoc-ld', action='store_true', default=False, help='Skip results from LD coefficient '
                                                                                'matrices calculated from the samples '
...
for par_tup, result_list in all_genes_dict.items():
    if not args.binary_assoc_test:
        ...
        par_str = f'prvl{prvl}_pqtl{pqtl}_alpha-{neg_alpha}_h2g{h2g}.{assoc_test}'

    logging.info(f'Merging {par_str} results ...')
    results = pd.concat(result_list, axis=1).T
    results_name = f'{args.out_dir}.{par_str}.{args.out_suffix}'
    results.to_csv(results_name, sep='\t')
    logging.info(f'Written {results_name}')

logging.info(f'Done 5.harvest_outputs.')
