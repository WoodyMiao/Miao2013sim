...
# Extract the first five genes for the test
head -n 5 ref_haplotypes/chr1.one32th.lst >ref_haplotypes/chr1.one32th.lst.head5
...
h2g=1e-2             # comma separated target heritability values
...
python_scripts/1.hapsim_a_population.py --nt 5 \
  ...
  --gene-list ref_haplotypes/chr1.one32th.lst.head5 \
  --out-dir 2.test_out
...
for method in kggsee ldak_sumher ldsc lder; do
  echo "Run $method ..."
  make --jobs $jobs --file 2.test_out/makefile.step1.$method
done
echo "Run gcta ..."
make -j 5 -f 2.test_out/makefile.step1.gcta
...
python_scripts/4.makefile_step2.py --jobs $jobs --chrom $chrom \
  ...
  --gene-list ref_haplotypes/chr1.one32th.lst.head5 \
  --out-dir $(pwd)/2.test_out # Here must use an absolute path, otherwise LDER will fail

for method in gcta kggsee hess ldak_gbat ldak_sumher ldsc lder; do
  echo "Run $method ..."
  sh 2.test_out/makefile.step2.$method.sh
done
...
python_scripts/5.harvest_outputs.py --nt 5 \
  ...
  --gene-list ref_haplotypes/chr1.one32th.lst.head5 \
  --out-suffix one32th.tsv --out-dir 2.test_out

# 6.plot_results
python_scripts/plot_scripts/2.repeat_quantitative_gcta.py --prefix 2.test_out \
