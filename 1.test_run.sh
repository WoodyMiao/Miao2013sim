...
for method in kggsee ldak_sumher ldsc lder; do
  echo "Run $method ..."
  make --jobs $jobs --file 1.test_out/makefile.step1.$method
done
...
python_scripts/4.makefile_step2.py --jobs $jobs --chrom $chrom \
  ...
  --gene-list ref_haplotypes/chr1.one4th.lst.head5concat \
  --out-dir $(pwd)/1.test_out # Here must use an absolute path, otherwise LDER will fail

for method in kggsee hess ldak_gbat ldak_sumher ldsc lder; do
  echo "Run $method ..."
  sh 1.test_out/makefile.step2.$method.sh
done
...
for q in q0-q1 q1-q2 q2-q3 q3-q4; do
  python_scripts/5.harvest_outputs.py --nt 5 \
    ...
    --out-suffix one4th_$q.tsv --out-dir 1.test_out
done

# 6.plot_results
python_scripts/plot_scripts/1.repeat_quantitative.py --prefix 1.test_out \
